...
async function handleCallback(req, res, code) {
  ...
  try {
    ...
    let tokenResponse;
    try {
      tokenResponse = await httpRequest('POST', 'https://oauth2.googleapis.com/token', {
      ...
      // Google OAuth codes are single-use. If a user refreshes/retries the
      // callback URL after a mid-flow failure (e.g. Outseta /tokens timed
      // out on the first attempt), Google rejects the already-consumed code
      // with invalid_grant. Show a clear "restart login" message instead of
      // the generic error page.
      if (isInvalidGrantError(err)) {
        console.warn('[GoogleSSO] invalid_grant on code exchange (likely retry of already-used code)');
...
async function generateOutsetaToken(email) {
  ...
  try {
    ...
    try {
      ...
      console.warn('[GoogleSSO] Outseta /tokens retry', { reason: err?.name || err?.message });
      tokenResponse = await call();
    }
